...
public class FoodDao {
	...
	public List<String>tipoDiPorzione(int c){
	...
	public List<InfoArco>getArchi(){
		String sql= "SELECT p1.portion_display_name AS n1, p2.portion_display_name AS n2, COUNT(DISTINCT(p1.food_code)) AS tot " + 
		...
		try {
			...
			while(res.next()) {
				try {
					InfoArco i=new InfoArco(res.getString("n1"),res.getString("n2"),res.getDouble("tot"));
					result.add(i);
				} catch (Throwable t) {
					t.printStackTrace();
				}
